    ...
    <form name='frm' id='frm' method='get' action='./list_by_memberno_search_paging.do'>
      ...
      <c:choose>
        ...
        <c:otherwise> <%-- 검색하지 않는 경우 --%>
          <input type='text' name='wantjob' id='wantjob' value='' style='width: 20%;'>
        </c:otherwise>
      </c:choose>
      <button type='submit' class="btn btn-secondary btn-sm">검색</button>
      <c:if test="${param.wantjob.length() > 0 }">
        <button type='button' class='btn btn-info btn-sm'
                     onclick="location.href='./list_by_memberno_search_paging.do?now_page=1&wantjob='">검색 취소</button>  
      </c:if>    
    </form>
  </DIV>
