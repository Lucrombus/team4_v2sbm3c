...
<c:set var="word" value="${eventVO.word }" />
<c:set var="size1_label" value="${eventVO.size1_label }" />
<c:set var="rdate" value="${eventVO.rdate.substring(0, 16) }" />
...
<A href="./list_all_search_paging.do?now_page=${param.now_page }" class='title_link'>이벤트</A> > 
<A href="./read.do?eventno=${eventno }&now_page=${param.now_page }&word=${param.word }" class='title_link'>${title }</A> </DIV>
...
<DIV class='content_body'>
  <ASIDE class="aside_right">
    ...
    <c:if test="${sessionScope.rankno == 1}">
      ...
      <A href="./update_text.do?eventno=${eventno}&now_page=${param.now_page}&word=${param.word }">글 수정</A>
      <span class='menu_divide' >│</span>
      <A href="./update_file.do?eventno=${eventno}&now_page=${param.now_page}">파일 수정</A>  
      <span class='menu_divide' >│</span>
      <A href="./delete.do?eventno=${eventno}&now_page=${param.now_page}">삭제</A>  
      <span class='menu_divide' >│</span>
    </c:if>
