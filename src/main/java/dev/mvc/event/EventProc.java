...
public class EventProc implements EventProcInter {
  ...
  public int search_count(EventVO eventVO) {
    int cnt = this.eventDAO.search_count(eventVO);
    return cnt;
  }
  ...
  public ArrayList<EventVO> list_all_search_paging(EventVO eventVO) {
    ...
    */
    int begin_of_page = (eventVO.getNow_page() - 1) * Event.RECORD_PER_PAGE;
    ...
    // 2 페이지 = 10 + 10: 20
    // 3 페이지 = 20 + 10: 30
    int end_num = begin_of_page + Event.RECORD_PER_PAGE;   
    /*
    ...
    */
    ...
    ArrayList<EventVO> list = this.eventDAO.list_all_search_paging(eventVO);
    
    for (EventVO vo : list) { // 특수 문자 처리
  ...
  public String pagingBox(int now_page, String word, String list_file){
    ...
    eventVO.setWord(word);
          
    int search_count = this.eventDAO.search_count(eventVO);  // 검색된 레코드 갯수 ->  전체 페이지 규모 파악
    int total_page = (int)(Math.ceil((double)search_count / Event.RECORD_PER_PAGE)); // 전체 페이지 수 
    int total_grp = (int)(Math.ceil((double)total_page / Event.PAGE_PER_BLOCK)); // 전체 그룹  수
    int now_grp = (int)(Math.ceil((double)now_page / Event.PAGE_PER_BLOCK));  // 현재 그룹 번호
    
    // 1 group: 1, 2, 3 ... 9, 10
    // 2 group: 11, 12 ... 19, 20
    // 3 group: 21, 22 ... 29, 30
    int start_page = ((now_grp - 1) * Event.PAGE_PER_BLOCK) + 1; // 특정 그룹의 시작  페이지  
    int end_page = (now_grp * Event.PAGE_PER_BLOCK);               // 특정 그룹의 마지막 페이지   
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    int _now_page = (now_grp - 1) * Event.PAGE_PER_BLOCK;  
    ...
    _now_page = (now_grp * Event.PAGE_PER_BLOCK)+1; //  최대 페이지수 + 1 
