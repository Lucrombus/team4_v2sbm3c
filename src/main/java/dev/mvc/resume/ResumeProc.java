...
public class ResumeProc implements ResumeProcInter {
  ...
  public int search_count(ResumeVO resumeVO) { 
    ...
    int cnt = this.resumeDAO.search_count(resumeVO); 
    return cnt;
  }
  ...
  public ArrayList<ResumeVO> list_by_memberno_search_paging(ResumeVO resumeVO) {
     ...
     */
    int begin_of_page = (resumeVO.getNow_page() - 1) * Resume.RECORD_PER_PAGE;
    ...
    // 2 페이지 = 0 + 20: 20
    // 3 페이지 = 0 + 30: 30
    int end_num = begin_of_page + Resume.RECORD_PER_PAGE;
    /*
     * 1 페이지: WHERE r >= 1 AND r <= 10 2 페이지: WHERE r >= 11 AND r <= 20 3 페이지: WHERE
     * r >= 21 AND r <= 30
     */
    resumeVO.setStart_num(start_num);
    resumeVO.setEnd_num(end_num);

    ArrayList<ResumeVO> list = this.resumeDAO.list_by_memberno_search_paging(resumeVO);
  ...
  public String pagingBox(int memberno, int now_page, String wantjob, String list_file) {
    ...
    int search_count = this.resumeDAO.search_count(resumeVO);
    int total_page = (int) (Math.ceil((double) search_count / Resume.RECORD_PER_PAGE)); // 전체 페이지 수
    int total_grp = (int) (Math.ceil((double) total_page / Resume.PAGE_PER_BLOCK)); // 전체 그룹 수
    int now_grp = (int) (Math.ceil((double) now_page / Resume.PAGE_PER_BLOCK)); // 현재 그룹 번호

    // 1 group: 1, 2, 3 ... 9, 10
    // 2 group: 11, 12 ... 19, 20
    // 3 group: 21, 22 ... 29, 30
    int start_page = ((now_grp - 1) * Resume.PAGE_PER_BLOCK) + 1; // 특정 그룹의 시작 페이지
    int end_page = (now_grp * Resume.PAGE_PER_BLOCK); // 특정 그룹의 마지막 페이지
...
//        str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    int _now_page = (now_grp - 1) * Resume.PAGE_PER_BLOCK;
    if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이 이상임으로 이전 그룹으로 갈수 있는 링크 생성
      str.append("<span class='span_box_1'><A href='" + list_file + "?&wantjob=" + wantjob + "&now_page=" + _now_page
          + "&memberno=" + memberno + "'>이전</A></span>");
    }
    ...
    for (int i = start_page; i <= end_page; i++) {
      ...
      if (now_page == i) { // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        str.append("<span class='span_box_1'><A href='" + list_file + "?wantjob=" + wantjob + "&now_page=" + i + "&memberno="
            + memberno + "'>" + i + "</A></span>");
      }
    }
    ...
    _now_page = (now_grp * Resume.PAGE_PER_BLOCK) + 1; // 최대 페이지수 + 1
    if (now_grp < total_grp) {
      str.append("<span class='span_box_1'><A href='" + list_file + "?&wantjob=" + wantjob + "&now_page=" + _now_page
          + "&memberno=" + memberno + "'>다음</A></span>");
    }
    str.append("</DIV>");
