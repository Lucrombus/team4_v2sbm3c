...
//  mav.addObject("fileName", file1saved);
  ...
  public ModelAndView read(int guin_cno, HttpServletRequest request) {
    ...
    String size1_label = Tool.unit(guin_cVO.getSize1());
    mav.addObject("size1_label", size1_label);
  ...
  /**
   * 시급별 추천목록
   * http://localhost:9093/guin_c/recommend_wage.do
   * 
   * @param typeno
   * @param word
   * @param now_page
  ...
  public ModelAndView recommend_wage(HttpSession session) {
    ModelAndView mav = new ModelAndView();
    ...
    // 검색 목록
    ArrayList<Guin_cVO> list_wage = guin_cProc.recommend_wage(recomVO.getJobcateno());
    mav.addObject("list_wage", list_wage);
