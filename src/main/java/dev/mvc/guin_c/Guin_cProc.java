...
import dev.mvc.recom.RecomDAOInter;
...
public class Guin_cProc implements Guin_cProcInter {
  ...
  public ArrayList<Guin_cVO> list_by_jobcateno_search_paging(Guin_cVO guin_cVO) {
     ...
     */
    int begin_of_page = (guin_cVO.getNow_page() - 1) * Guin_c.RECORD_PER_PAGE;
    ...
    // 2 페이지 = 0 + 20: 20
    // 3 페이지 = 0 + 30: 30
    int end_num = begin_of_page + Guin_c.RECORD_PER_PAGE;
    /*
  ...
  public String pagingBox(int jobcateno, int search_count, int now_page, String word, String list_file) {
    ...
    int total_page = (int) (Math.ceil((double) search_count / Guin_c.RECORD_PER_PAGE)); // 전체 페이지 수
    int total_grp = (int) (Math.ceil((double) total_page / Guin_c.PAGE_PER_BLOCK)); // 전체 그룹 수
    int now_grp = (int) (Math.ceil((double) now_page / Guin_c.PAGE_PER_BLOCK)); // 현재 그룹 번호

    // 1 group: 1, 2, 3 ... 9, 10
    // 2 group: 11, 12 ... 19, 20
    // 3 group: 21, 22 ... 29, 30
    int start_page = ((now_grp - 1) * Guin_c.PAGE_PER_BLOCK) + 1; // 특정 그룹의 시작 페이지
    int end_page = (now_grp * Guin_c.PAGE_PER_BLOCK); // 특정 그룹의 마지막 페이지
...
//        str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    int _now_page = (now_grp - 1) * Guin_c.PAGE_PER_BLOCK;
    ...
    _now_page = (now_grp * Guin_c.PAGE_PER_BLOCK) + 1; // 최대 페이지수 + 1
  ...
  @Override
  public ArrayList<Guin_cVO> recommend_wage(int jobcateno) {
    ArrayList<Guin_cVO> list_wage = this.guin_cDAO.recommend_wage(jobcateno);  
